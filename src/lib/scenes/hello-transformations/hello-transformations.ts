...
const helloTransformations: RenderWrapper = (canvas, shaderSources) => {
  ...
  function render(
    ...
    setUniform(gl, sceneObject.shaderProgram, {
      name: "transform",
      type: "mat4-float",
      value: rotatingContainerTransform
    });
    ...
    const scaleAmount = (Math.sin(timeInSeconds) + 1) * 0.5 + 0.5;
    mat4.scale(
      scalingContainerTransform,
      scalingContainerTransform,
      vec3.fromValues(scaleAmount, scaleAmount, 1.0)
    );

    setUniform(gl, sceneObject.shaderProgram, {
      name: "transform",
      type: "mat4-float",
      value: scalingContainerTransform
    });
...
export default helloTransformations;
