...
// Shaders
import containerVertexShaderSource from "~/lib/scenes/materials/container.vert?raw";
import containerFragmentShaderSource from "~/lib/scenes/materials/container.frag?raw";
import lightVertexShaderSource from "~/lib/scenes/materials/light.vert?raw";
import lightFragmentShaderSource from "~/lib/scenes/materials/light.frag?raw";

const materials: RenderWrapper = (canvas) => {
  ...
  function render(gl: WebGL2RenderingContext, canvas: HTMLCanvasElement, currentTime: number = 0) {
    ...
    lightColour = vec3.fromValues(
      Math.sin(currentTime * 0.001 * 2.0),
      Math.sin(currentTime * 0.001 * 0.7),
      Math.sin(currentTime * 0.001 * 1.3)
    );
